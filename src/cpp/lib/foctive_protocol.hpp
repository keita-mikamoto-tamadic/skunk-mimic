...
namespace Foctive {
  ...
  enum class SettingsCmd : uint8_t {
    ...
    kAnyValPosOffset    = 110,  // 現在位置を任意の値として設定
  ...
  inline void MakeAnyValPosOffset(uint8_t device_id, float target_pos, CanFdFrame& out) {
    StartSettingsFrame(SettingsCmd::kAnyValPosOffset, device_id, out);
  ...
  inline SettingsReply ParseSettings(const CanFdFrame& frame, MotParam& param) {
    ...
    switch (r.cmd) {
      ...
      case SettingsCmd::kAnyValPosOffset:   // 110: [cmd, offset(float)] (適用 offset)
